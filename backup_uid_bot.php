...
if (isset($update["message"])) {
    ...
    if (isset($message["reply_to_message"])) {
        $isAdmin = get_admin_group($botToken, $groupId_nhan, $update);
        if($isAdmin == true) {
            try {
                // Tạo kết nối
                $conn = new mysqli($servername, $username, $password, $dbname);
                $result_admin = get_admin_database($conn, $update);
                if ($result_admin > 0) {
                    try {
                        $content = get_user_database($idReplyUser, $conn);
                        $replyToMessageId = $message["message_id"];
                    }
                    catch(PDOException $e) {
                        $content = "Lỗi ở Reply_to_message (1): " . $e->getMessage();
                    }
                }
                // Đóng kết nối
                $conn->close();
                ...
                $content = "Lỗi ở Reply_to_message (2): " . $e->getMessage();
            }
            ...
            if ($message["text"] == "uid") {
                ...
                $response = 'Lệnh này không có, vui lòng kiểm tra lại';
            }
        }
    }
